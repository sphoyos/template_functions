...
postfile tall_cnt  talli  veros val_p hri li_hri ls_hri hrj li_hrj ls_hrj using `mincnt'
...
forvalues i = 3(1)`nobsi' {
local inij=`i'+3
forvalues j = `inij' (1)`nobsj' { 
 
disp  in yellow " `i' ,    `vartalli'[`i']  "
disp  in yellow " `j' ,    `vartalli'[`j']  "
...
qui replace r_`vartallj'=0  if `vartalli'!=.
local talli= `vartalli'[`i']
local tallj= `vartalli'[`j']
**** Si el OR de la variable continua es <1 l'indicador valdra 1 per valors per damunt (>) Si es mes gran cal posar >*******
qui replace r_`vartalli'=1 if `vartalli'> `talli' & `vartalli'< `tallj'  & `vartalli'!=.
qui replace r_`vartallj'=1 if  `vartalli'>= `tallj'  & `vartalli'!=.


************************************************ CAL CANVIAR OOOJOOOOOO   ****************************************
**** Cal escriure el model de regressió canviant les variables cuantitatives pels indicadors creats r_vari

qui xi: `tiporeg'  `var_resp'  `var_mod'   r_`vartalli' r_`vartallj'

local veros=e(ll)
local hri=exp(_b[r_`vartalli'])
local li_hri=exp(_b[r_`vartalli']-invnorm(0.975)*_se[r_`vartalli'])
local ls_hri=exp(_b[r_`vartalli']+invnorm(0.975)*_se[r_`vartalli'])
local hrj=exp(_b[r_`vartallj'])
local lij_hr=exp(_b[r_`vartallj']-invnorm(0.975)*_se[r_`vartallj'])
local lsj_hr=exp(_b[r_`vartallj']+invnorm(0.975)*_se[r_`vartallj'])
*** Es guarden al fixter temporal els punts de tall i la versemblança
lrtest mod0
local val_p=   r(p)

post tall_cnt  (`talli') (`veros') (`val_p')  (`hri') (`li_hri') (`ls_hri') (`hrj') (`li_hrj') (`ls_hrj')
}
*}
